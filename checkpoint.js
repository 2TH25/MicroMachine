    ...
    estEnColision(vehicule){
        for (let i=0;i<this.itBox.length;i++){
            for (let j=0;j<vehicule.itBox.length;j++){
                dx = this.itBox[i].x - (vehicule.itBox[j].x + vehicule.position.x)
                dy = this.itBox[i].y - (vehicule.itBox[j].y + vehicule.position.y)
                distance = Math.sqrt(dx * dx + dy * dy)
                if (distance < this.itBox[i].r + itBoxVehic.r){
